...
if(isset($_REQUEST['aeid']))
{
    ...
    // Recupera i dati del noleggio da confermare
    $sql = "SELECT IDveicolo, dataI, dataF FROM tblnoleggi WHERE id = :aeid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':aeid', $aeid, PDO::PARAM_INT);
    $query->execute();
    $noleggio = $query->fetch(PDO::FETCH_OBJ);

    if ($noleggio) {
        $idVeicolo = $noleggio->IDveicolo;
        $dataI = $noleggio->dataI;
        $dataF = $noleggio->dataF;

        // Controlla se esiste già un noleggio confermato per lo stesso veicolo e periodo sovrapposto
        $sqlCheck = "SELECT COUNT(*) FROM tblnoleggi 
                     WHERE IDveicolo = :idVeicolo 
                       AND Status = 1 
                       AND id != :aeid
                       AND (
                            (dataI <= :dataF AND dataF >= :dataI)
                        )";

        $queryCheck = $dbh->prepare($sqlCheck);
        $queryCheck->bindParam(':idVeicolo', $idVeicolo, PDO::PARAM_INT);
        $queryCheck->bindParam(':dataI', $dataI);
        $queryCheck->bindParam(':dataF', $dataF);
        $queryCheck->bindParam(':aeid', $aeid, PDO::PARAM_INT);
        $queryCheck->execute();
        $conflitti = $queryCheck->fetchColumn();

        if ($conflitti > 0) {
            $error = "Attenzione: Esiste già un noleggio confermato per questo veicolo nel periodo selezionato.";
        } else {
            // Conferma il noleggio
            $status = 1;
            $sqlUpdate = "UPDATE tblnoleggi SET Status = :status WHERE id = :aeid";
            $queryUpdate = $dbh->prepare($sqlUpdate);
            $queryUpdate->bindParam(':status', $status, PDO::PARAM_INT);
            $queryUpdate->bindParam(':aeid', $aeid, PDO::PARAM_INT);
            $queryUpdate->execute();

            $msg = "Noleggio confermato con successo";
        }
    } else {
        $error = "Errore: richiesta di noleggio non trovata.";
    }
}
